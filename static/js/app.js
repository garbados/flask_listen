...
$(function(){
  ...
  function ready(error, world, names, counts_rows) {
        ...
        // our list of countries, derived from the topojson
    var countries = topojson.object(world, world.objects.countries).geometries,
        // dict for storing country names and their tweet counts
        counts = {},
        // the most tweets, which will correspond to entirely blue
        most = Math.max.apply(null, counts_rows.rows.map(function(row){ 
          return row.value
        })),
        // color scale: logarithmically proceed from black to blue
        // based on tweet count
        color = d3.scale.log().domain([1, most]).range(['black', 'blue']);

    // map counts to country names
